...
public:
    ...
    void OnPlayerLogin(Player* player) override
    {
        ...
        else // normal account
        {
            ...
            if ((player->getRace() == RACE_GOBLIN || player->getRace() == RACE_WORGEN) && sIndividualProgression->cataRacesStartingProgression && !sIndividualProgression->hasPassedProgression(player, static_cast<ProgressionState>(sIndividualProgression->cataRacesStartingProgression)))
            {
    ...
    {
        if (!sIndividualProgression->enabled || !player || player->IsDuringRemoveFromWorld() || !player->IsInWorld())
            return;
        ...
        if (master && master != player && master->IsInWorld())
        {
            ...
            ProgressionState botState = sIndividualProgression->GetCurrentProgressionState(player);

            if (botState != masterState)
        ...
        ContentLevels mapEntryExpansion = GetContentLevelsForMapAndZone(player->GetMapId(), player->GetZoneId());

        if (level >= 81 || mapEntryExpansion == CONTENT_81_90)
        {
            progressionState = PROGRESSION_CATA_TIER_4;
            validAuras = { 98654, 98655, 98656, 98657, 98658 };
        }
        else if ((level >= 70 && level <= 80) || mapEntryExpansion == CONTENT_71_80)
        {
            progressionState = PROGRESSION_WOTLK_TIER_4;
            validAuras = { 98649, 98650, 98651, 98652, 98653 };
        }
        else if ((level >= 60 && level <= 69) || mapEntryExpansion == CONTENT_61_70)
        {
        ...
        bool hasValidAura = std::any_of(validAuras.begin(), validAuras.end(), [player](uint32 auraId)
        {
    ...
    {
        if (!sIndividualProgression->enabled || !player || !player->GetMap())
            return;
        ...
        if (!sIndividualProgression->hasPassedProgression(player, PROGRESSION_PRE_TBC))
            multiplier = sIndividualProgression->vanillaHealthAdjustment;
        else if (!sIndividualProgression->hasPassedProgression(player, PROGRESSION_TBC_TIER_5))
            multiplier = sIndividualProgression->tbcHealthAdjustment;
    ...
    void OnPlayerAchievementComplete(Player* player, AchievementEntry const* achievement) override
    {
        ...
        uint32 eventId = 0;
        ProgressionState progression = PROGRESSION_START;

        switch (achievement->ID)
        {
        case HALION_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_RUBY_SANCTUM;
            progression = PROGRESSION_WOTLK_TIER_5;
            break;
        case LICH_KING_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_ICC;
            progression = PROGRESSION_WOTLK_TIER_4;
            break;
        case ANUB_ARAK_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_TOTC;
            progression = PROGRESSION_WOTLK_TIER_3;
            break;
        case KEL_THUZAD_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_NAXXRAMAS;
            progression = PROGRESSION_WOTLK_TIER_1;
            break;
        case KIL_JAEDEN_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_SUNWELL;
            progression = PROGRESSION_TBC_TIER_5;
            break;
        case ILLIDAN_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_BLACK_TEMPLE;
            progression = PROGRESSION_TBC_TIER_4;
            break;
        case ZUL_JIN_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_ZUL_AMAN;
            progression = PROGRESSION_TBC_TIER_4;
            break;
        case KAEL_THAS_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_TEMPEST_KEEP;
            progression = PROGRESSION_TBC_TIER_2;
            break;
        case MALCHEZAAR_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_KARAZHAN;
            progression = PROGRESSION_TBC_TIER_1;
            break;
        case KEL_THUZAD_40_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_NAXX40;
            progression = PROGRESSION_NAXX40;
            break;
        case C_THUN_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_AQ;
            progression = PROGRESSION_AQ;
            break;
        case NEFARIAN_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_BLACKWING_LAIR;
            progression = PROGRESSION_BLACKWING_LAIR;
            break;
        case ONYXIA_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_ONYXIA;
            progression = PROGRESSION_ONYXIA;
            break;
        case RAGNAROS_KILL:
            eventId = ACHIEVEMENT_SPELL_EVENT_MOLTEN_CORE;
            progression = PROGRESSION_MOLTEN_CORE;
            break;
        default:
            return;
        }

        player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, eventId);
        sIndividualProgression->UpdateProgressionState(player, progression);
    }
    ...
    void OnPlayerUpdateZone(Player* player, uint32 /*newZone*/, uint32 newArea) override
    {
        ...
        if (!player->GetPlayerSetting("TBC-MOVIE", 0).value &&
        ...
        if (!player->GetPlayerSetting("WOTLK-MOVIE", 0).value &&
    ...
    void OnPlayerUpdateArea(Player* player, uint32 oldArea, uint32 newArea) override
    {
        ...
        case AREA_ICECROWN_CITADEL_OUTSIDE:
            if (sIndividualProgression->hasPassedProgression(player, PROGRESSION_WOTLK_TIER_3) &&
                !player->GetPlayerSetting("FOTLK-MOVIE", 0).value)
            {
                player->SendMovieStart(MOVIE_FOTLK);
                player->UpdatePlayerSetting("FOTLK-MOVIE", 0, 1);
            }
            break;
        default:
            break;
...
public:
    ...
    bool CanAccountCreateCharacter(uint32 accountId, uint8 charRace, uint8 charClass) override
    {
        ...
        if ((!sIndividualProgression->enabled) ||
            ...
            (!sIndividualProgression->tbcRacesProgressionLevel && !sIndividualProgression->cataRacesProgressionLevel && !sIndividualProgression->deathKnightProgressionLevel))
        {
            return true;
        }
        ...
        if ((charRace == RACE_GOBLIN || charRace == RACE_WORGEN) && sIndividualProgression->cataRacesProgressionLevel != 0)
        {
