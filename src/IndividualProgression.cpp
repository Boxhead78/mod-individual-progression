...
namespace
{
    void UpdatePlayerMaxLevel(Player* player)
    {
        if (!player)
            return;
...
ProgressionState IndividualProgression::GetCurrentProgressionState(Player* player) const
{
    ...
    return static_cast<ProgressionState>(player->GetPlayerSetting("mod-individual-progression", SETTING_PROGRESSION_STATE).value);
}

uint8 IndividualProgression::GetPlayerProgressionFromQuests(Player* player) const
{
    return static_cast<uint8>(GetCurrentProgressionState(player));
}
...
void IndividualProgression::UpdateProgressionState(Player* player, ProgressionState newState) const
{
    ...
    if (newState > currentState)
    {
        ...
        removeAllProgressionSpells(player);
        setProgressionSpell(player, newState);
        UpdateProgressionQuests(player);
    }

    UpdatePlayerMaxLevel(player);
}
...
void IndividualProgression::UpdateProgressionQuests(Player* player) const
{
    if (!player || !player->IsInWorld())
        return;

    ProgressionState currentState = GetCurrentProgressionState(player);

    // These hidden quests are only a compatibility mirror for SQL conditions.
    // PlayerSettings remains the authoritative progression source.
    for (uint8 state = PROGRESSION_MOLTEN_CORE; state <= PROGRESSION_WOTLK_TIER_5; ++state)
    {
        uint32 questId = 66000 + state;
        bool shouldBeRewarded = currentState >= state;
        bool isRewarded = player->GetQuestStatus(questId) == QUEST_STATUS_REWARDED;

        if (!shouldBeRewarded && isRewarded)
        {
...
// Return the highest PlayerSettings progression level achieved by an account.
uint8 IndividualProgression::GetAccountProgression(uint32 accountId)
{
    ...
    if (result)
    {
        do
        {
            ...
            dataString >> firstValue;
            uint8 value = static_cast<uint8>(std::atoi(firstValue.c_str()));
            if (value > progressionLevel)
...
{
    // This fork intentionally has no account-based progression exceptions.
    return false;
}
...
bool IndividualProgression::isNormalAccount(Player* player)
{
    ...
    return !sIndividualProgression->isBotAccount(player);
}
...
void IndividualProgression::setProgressionSpell(Player* player, ProgressionState newState) const
{
    if (!player || player->IsDuringRemoveFromWorld() || !player->IsInWorld())
        return;

    player->CastSpell(player, 98636 + static_cast<uint32>(newState), true);
}

void IndividualProgression::removeAllProgressionSpells(Player* player) const
{
    if (!player || player->IsDuringRemoveFromWorld() || !player->IsInWorld())
        return;

    for (uint32 spellId = 98636; spellId <= 98636 + PROGRESSION_CATA_TIER_4; ++spellId)
    {
...
private:
    static void LoadConfig()
    {
        ...
        sIndividualProgression->vanillaPowerAdjustment = sConfigMgr->GetOption<float>("IndividualProgression.VanillaPowerAdjustment", 0.5f);
        sIndividualProgression->vanillaHealingAdjustment = sConfigMgr->GetOption<float>("IndividualProgression.VanillaHealingAdjustment", 0.5f);
        sIndividualProgression->vanillaHealthAdjustment = sConfigMgr->GetOption<float>("IndividualProgression.VanillaHealthAdjustment", 0.5f);
        sIndividualProgression->tbcPowerAdjustment = sConfigMgr->GetOption<float>("IndividualProgression.TBCPowerAdjustment", 0.7f);
        sIndividualProgression->tbcHealingAdjustment = sConfigMgr->GetOption<float>("IndividualProgression.TBCHealingAdjustment", 0.7f);
        sIndividualProgression->tbcHealthAdjustment = sConfigMgr->GetOption<float>("IndividualProgression.TBCHealthAdjustment", 0.7f);
        sIndividualProgression->questXpFix = sConfigMgr->GetOption<bool>("IndividualProgression.QuestXPFix", false);
        ...
        sIndividualProgression->fishingFix = sConfigMgr->GetOption<bool>("IndividualProgression.FishingFix", false);
        ...
        sIndividualProgression->simpleConfigOverride = sConfigMgr->GetOption<bool>("IndividualProgression.SimpleConfigOverride", false);
        sIndividualProgression->progressionLimit = sConfigMgr->GetOption<uint8>("IndividualProgression.ProgressionLimit", 17);
        ...
        sIndividualProgression->repeatableVanillaQuestsXp = sConfigMgr->GetOption<bool>("IndividualProgression.RepeatableVanillaQuestsXP", false);
        ...
        sIndividualProgression->tbcRacesProgressionLevel = sConfigMgr->GetOption<uint8>("IndividualProgression.TbcRacesUnlockProgression", 6);
        sIndividualProgression->tbcRacesStartingProgression = sConfigMgr->GetOption<uint8>("IndividualProgression.tbcRacesStartingProgression",
            sConfigMgr->GetOption<uint8>("IndividualProgression.TbcRacesStartingProgression", 0));
        sIndividualProgression->cataRacesProgressionLevel = sConfigMgr->GetOption<uint8>("IndividualProgression.CataRacesUnlockProgression", 17);
        sIndividualProgression->cataRacesStartingProgression = sConfigMgr->GetOption<uint8>("IndividualProgression.CataRacesStartingProgression", 0);
        ...
        sIndividualProgression->VanillaPvpTitlesEarnPostVanilla = sConfigMgr->GetOption<bool>("IndividualProgression.VanillaPvpEarnTitlesAfterVanilla", true);
        ...
        sIndividualProgression->progressionSetterAlwaysVisible = sConfigMgr->GetOption<bool>("IndividualProgression.ProgressionSetterAlwaysVisible", true);
    }
